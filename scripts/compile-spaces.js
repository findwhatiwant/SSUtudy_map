...
async function compileFromLocal() {
  console.log('Compiling from local markdown files in src/content/spaces...')
  const spacesDir = path.resolve('src/content/spaces')
  if (!fs.existsSync(spacesDir)) {
    console.warn('src/content/spaces directory does not exist.')
    return []
  }
  const files = fs.readdirSync(spacesDir).filter(f => f.endsWith('.md'))
  const spaces = []
  for (const file of files) {
    const raw = fs.readFileSync(path.join(spacesDir, file), 'utf-8')
...
async function main() {
  ...
  if (projectId) {
    console.log(`Attempting to fetch spaces from Firestore project: ${projectId}...`)
    ...
    try {
      ...
      console.warn('Error fetching from Firestore, falling back to local:', error.message)
    }
  }

  // Firestore 결과가 없거나 실패 시 로컬 마크다운 컴파일로 복구
  if (spaces.length === 0) {
    spaces = await compileFromLocal()
  }
  ...
  console.log(`Successfully compiled ${spaces.length} spaces to public/spaces.json`)
}
