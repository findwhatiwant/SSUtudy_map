...
- [x] **정적 빌드 파일 컴파일 및 런타임 페치(Option 2) 구현 완료 (서버 읽기 비용 0원 달성)**
...
## 8. Firestore 데이터 읽기 비용 0원 최적화 (Option 2 정적 파일 컴파일 빌드)
- **문제**: 지도 로딩 시마다 Firestore에 직접 수백~수천 개의 문서를 조회하게 되면 트래픽 급증 시 Firebase 무료 쿼터(일일 5만 회 읽기)를 순식간에 초과하여 과금이 발생하게 됨.
- **해결**: 빌드 타임에 Node.js 스크립트([compile-spaces.js](file:///Users/jousig/programming/SSU_study_map/scripts/compile-spaces.js))가 Firestore의 `spaces` 컬렉션을 REST API로 통째로 긁어와 `spaces.json` 정적 파일로 사전에 빌드/컴파일 해두는 구조를 수립. 
- **런타임**: 클라이언트([studySpaceRepository.ts](file:///Users/jousig/programming/SSU_study_map/src/models/studySpaceRepository.ts))는 이 `./spaces.json`을 단 1회의 비동기 HTTP 요청으로 불러와 마크업 처리하며, 캐싱으로 인한 화면 노출 지연 방지를 위해 타임스탬프 기반 캐시 버스터(`?t=Date.now()`)를 연동하여 완전한 무료 운영 가능 구조 구축.

## 9. 숭실대학교 캠퍼스 핀 좌표 불일치 정밀 교정 (OSM 연동)
- **문제**: 공부 공간들의 지도 핀이 실제 건물 옥상이나 정확한 입구가 아닌, 엉뚱한 길바닥이나 이웃 건물(예: 학생회관이 형남공학관 자리로 가는 등)에 엇갈려 표출되던 상태 확인.
- **해결**: 숭실대학교 캠퍼스 영역의 OpenStreetMap(OSM) XML 데이터를 통째로 다운로드받아 빌딩 외곽 노드들의 위경도 값을 정밀 파싱하고 평균 중심점(Centroid)을 재계산. 형남공학관(`37.49586, 126.95594`), 학생회관(`37.49699, 126.95640`) 등 6개 핵심 건물의 실제 중심 좌표를 복구해 로컬 마크다운(.md) 메타데이터에 기입 완료.
- **예외 처리**: 빌드 컴파일 스크립트에 `CORRECT_COORDS` 매핑 오버라이드 테이블을 두어 DB 데이터 누락 시에도 핀들이 무조건 정확한 건물 위에 달라붙도록 안전 설계.

## 10. 로컬 제보 관리 전용 비공개 CLI 검토기 구축
- **기능**: 사용자들이 프론트엔드 모달을 통해 지도 핀을 꽂고 제보한 Firestore `suggestions` 대기 컬렉션 데이터를 로컬 터미널에서 대화형으로 심사하고 승인 처리해 주는 관리 스크립트([suggestions-to-md.local.js](file:///Users/jousig/programming/SSU_study_map/scripts/suggestions-to-md.local.js)) 개발.
- **동작**: 터미널에서 한 건씩 정보를 확인하고 `y`를 누르면 파일 ID(slug)를 지정해 `src/content/spaces/*.md` 파일로 자동 추출 생성해주고 Firestore의 상태값을 `approved`로 전송.
- **보안**: 해당 로컬 스크립트는 깃허브 원격 퍼블릭 저장소에 노출되지 않도록 `.gitignore`에 `scripts/*.local.js` 예외 필터를 적용하여 보안 유지.
